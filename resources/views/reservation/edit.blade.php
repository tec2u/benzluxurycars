...
@section('content')
        ...
        <div class="flex justify-between md:flex-row flex-col ">
            ...
            <div class="md:w-2/3  md:border-r border-gray-800 p-2">
                ...
                <div class="px-6 md:me-8">
                    <form id="reservation_form" action="{{ route('car.reservationUpdate', ['reservation_id' => $reservation->id]) }}"
                        method="POST">
                        @csrf
                        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-full">
                                ...
                                <x-flatpickr range id="laravel-flatpickr" name="reservation_dates" class="w-full rounded-md"
                                    placeholder="Reservation Dates" value="{{ $reservation->start_date }} to {{ $reservation->end_date }}"/>
                            </div>
                        </div>
                        <div class="mt-12 md:block hidden  ">
                            <button type="submit"
                                class="text-white bg-pr-400 p-3 w-full rounded-lg font-bold hover:bg-black shadow-xl hover:shadow-none ">Save changes</button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#duration span').text('{{ $reservation->days }}' + ' days');
            $('#total-price span').text('{{ intval($reservation->total_price) }}' + ' AED');
